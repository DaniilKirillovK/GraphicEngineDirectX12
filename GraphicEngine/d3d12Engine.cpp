...
MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	return D3D12Engine::GetApp()->MsgProc(hwnd, msg, wParam, lParam);
}

D3D12Engine* D3D12Engine::mApp = nullptr;
D3D12Engine* D3D12Engine::GetApp()
{
	return mApp;
}

D3D12Engine::D3D12Engine(HINSTANCE hInstance)
	: mhAppInst(hInstance)
{
...
HINSTANCE D3D12Engine::AppInst()const
{
	return mhAppInst;
}

HWND D3D12Engine::MainWnd()const
{
	return mhMainWnd;
}

float D3D12Engine::AspectRatio()const
{
	return static_cast<float>(mClientWidth) / mClientHeight;
}

bool D3D12Engine::Get4xMsaaState()const
{
	return m4xMsaaState;
}

void D3D12Engine::Set4xMsaaState(bool value)
{
...
void D3D12Engine::OnResize()
{
	...
	//auto tmp2 = CD3DX12_RESOURCE_BARRIER::Transition(mDepthStencilBuffer.Get(),
	//	D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_DEPTH_WRITE);
	//mCommandList->ResourceBarrier(1, &tmp2);
...
bool D3D12Engine::InitMainWindow()
{
	WNDCLASS wc;
	...
	mhMainWnd = CreateWindow(L"MainWnd", mMainWndCaption.c_str(),
		WS_OVERLAPPEDWINDOW | WM_GETMINMAXINFO, CW_USEDEFAULT, CW_USEDEFAULT, width, height, 0, 0, mhAppInst, 0);
	if (!mhMainWnd)
	{
...
GeometryManager::BuildScene3LODGeometry(mCommandList, md3dDevice);
GeometryManager::BuildScene4Geometry(mCommandList, md3dDevice);
GeometryManager::BuildScene5Geometry(mCommandList, md3dDevice);
GeometryManager::BuildScene6Geometry(mCommandList, md3dDevice);
GeometryManager::BuildScene7Geometry(mCommandList, md3dDevice);
GeometryManager::BuildScene9Geometry(mCommandList, md3dDevice);
GeometryManager::BuildScene9RMDemoGeometry(mCommandList, md3dDevice);
GeometryManager::BuildModelsGeometry(mCommandList, md3dDevice);
GeometryManager::BuildScene10DebugGeometry(mCommandList, md3dDevice);
GeometryManager::BuildScene10MoreLightGeometry(mCommandList, md3dDevice);
...
ID3D12Resource* D3D12Engine::CurrentBackBuffer()const
{
...
ID3D12Resource* D3D12Engine::Scene13RenderTargetBuffer() const
{
	return mRenderTargetBufferScene13.Get();
}
...
void D3D12Engine::LogAdapterOutputs(IDXGIAdapter* adapter)
{
	UINT i = 0;
...
void D3D12Engine::LogOutputDisplayModes(IDXGIOutput* output, DXGI_FORMAT format)
{
	UINT count = 0;
	UINT flags = 0;
