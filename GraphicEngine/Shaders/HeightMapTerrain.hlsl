...
Texture2D<float4> gTextures[2] : register(t0, space0);
...
cbuffer cbTessHeightMap : register(b3)
{
    float gTessFactor;
...
VertexOut VS(VertexIn vin, uint instanceID : SV_InstanceID)
{
    ...
    float4 texC = mul(float4(vin.TexC * 100, 0.0f, 1.0f), gTexTransform);
...
PatchTess ConstantHS(InputPatch<VertexOut, 3> patch, uint patchID : SV_PrimitiveID)
{
    ...
    // may still be visible with dot product = 0
    {
        ...
        const float d0 = 5.0f;
        const float d1 = 60.0f;
        float tess = 64.0f;
        if (bIsDistantAdaptiveTess == 1)
        {
            tess = gTessFactor * pow(saturate((d1 - d) / (d1 - d0)), 2.0f);
        }
             ...
             const OutputPatch<HullOut, 3> tri)
{
    ...
    float displacement = gTextures[1].SampleLevel(gsamLinearWrap, texCoord / 1000, 0).r;
