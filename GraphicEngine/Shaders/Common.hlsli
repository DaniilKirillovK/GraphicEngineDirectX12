...
ConstantBuffer<Constants> ConstantsBuffer : register(b0);
ConstantBuffer<DrawParams> DrawParamsBuffer : register(b1);

ConstantBuffer<MeshInfo>   MeshInfoBuffer[MAX_LOD_LEVELS] : register(b2);
...
bool IsVisible(float4 boundingSphere)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        if (dot(center, ConstantsBuffer.Planes[i]) < -radius)
        {
            return false;
        }
    }
...
{
    float3 v = boundingSphere.xyz - ConstantsBuffer.ViewPosition;
    float r = boundingSphere.w;

    // Sphere radius in screen space
    float size = ConstantsBuffer.RecipTanHalfFovy * r / sqrt(dot(v, v) - r * r);
    size = min(size, 1.0);

    return (1.0 - size) * (ConstantsBuffer.LODCount - 1);
}
