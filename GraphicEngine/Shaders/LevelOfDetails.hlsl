...
GBuffer PS(DomainOut pin)
{
    ...
    if (gLevelOfDetail == 0)
        selectedSampler = gsamLevel0;
    else if (gLevelOfDetail == 1)
        selectedSampler = gsamLevel1;
    else if (gLevelOfDetail == 2)
        selectedSampler = gsamLevel2;
    else if (gLevelOfDetail == 3)
        selectedSampler = gsamLevel3;
        
    float2 texCoord = pin.TexC;
    gBuffer.Albedo = gTextures[0].Sample(selectedSampler, texCoord) * gDiffuseAlbedo * pin.Color;
